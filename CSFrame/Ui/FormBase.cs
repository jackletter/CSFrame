...
namespace CSFrame.Ui
{
    ...
    {
        protected CefCustomObject cef;
        protected ChromiumWebBrowser chromeBrowser;
        public Dictionary<string, string> eventHandlers = new Dictionary<string, string>();
        ...
        {
            NativeMethods.SendMessage(Handle, (int)WindowMessages.WM_SYSMENU, 0, MakeLong((short)(pos.X + Location.X), (short)(pos.Y + Location.Y)));
        }
        ...
        {
            return Location.X == -8 && Location.Y == -8 && Size.Width == (MaximumSize.Width + 16) && Size.Height == (MaximumSize.Height + 16);
        }
        ...
        protected void WmWindowPosChanged(ref Message m)
        {
            DefWndProc(ref m);
            UpdateBounds();
            m.Result = NativeConstants.TRUE;
            if (cef != null && chromeBrowser != null)
            {
                //判断当前是否全屏
                if (IsFullScreen())
                {
                    if (eventHandlers.ContainsKey("onToggleWindowState"))
                    {
                        chromeBrowser.ExecuteScriptAsync(eventHandlers["onToggleWindowState"], true, m.Msg.ToString("X"));
                    }
                }
                else
                {
                    if (eventHandlers.ContainsKey("onToggleWindowState"))
                    {
                        chromeBrowser.ExecuteScriptAsync(eventHandlers["onToggleWindowState"], false, m.Msg.ToString("X"));
                    }
                }
            }
        }
        ...
        protected void WmNCPaint(ref Message m)
        {
        ...
        protected void WmSetText(ref Message msg)
        {
        ...
        protected void WmNCActivate(ref Message msg)
        {
